...
class LoggerDecorate(BaseDecorate):
    ...
    def __call__(self, *args, **kwds):
        with open('./PR24/log.txt', 'a+', encoding='utf-8') as f:
...
def time_decorator(func):
    '''Функция-декоратор, которая заменяет класс-декоратор decorators.TimeDecorate'''
    def wrapper(*args, **kwgs):
        start = time.time()
        result = func(*args, **kwgs)
        end = time.time()
        print(f'Функция {func} отработала за {(end - start):.8f} сек.')
        return result
    return wrapper
